Cookies are managed by two variables available in the scripts :
  ...
  * SET\_COOKIE is a dictionary-like object used by the script to set cookies values, path, expiry date etc. and send them to the browser
...
This form is submitted to the function set\_cookie(name,value). In this function, a new entry is created in SET\_COOKIE with the submitted data. The key 'path' of this entry is set to the server root. Then a redirection is performed to the index() function
...
To erase the cookie, a link is provided, leading to the function erase(name) where name is the cookie name. The function erase() uses SET\_COOKIE to erase the value, and set the expiry date to the past (10 days before today), using the function cookie\_expiry\_date() to transform a datetime object to the format supported by the cookie specifications
